...
| 2 个 Key | Key1 开始录音 / Key2 停止录音 |

**通信：** MQTT（BearPi-Pico H3863 ↔ 云服务器）

**服务器：** 华为云 ECS（Ubuntu 20.04.6 LTS / 2 核 2.6GHz / 3.7GB RAM）
...
| 项目 | 选择 | 原因 |
|------|------|------|
| 服务器语言 | Python 3.8.10 | 已部署在服务器，MQTT 库成熟、AI API SDK 完善、音频处理方便 |
| 音频格式 | 16kHz/16bit/单声道 | WM8978G 常用配置 |
| MQTT Broker | EMQX | 已在服务器部署，性能好、功能丰富 |
| ASR 平台 | 讯飞 | 需满足兼容性要求 |
...
Python 3.8 已 EOL，需锁定依赖版本避免自动升级到不兼容版本：
...
> **注意**：`openai>=1.30` 已放弃 Python 3.8 支持，必须使用 `<1.30` 的版本。实际部署前应测试确认所选版本在 3.8.10 上可正常安装和运行。所有依赖需在 `requirements.txt` 中锁定版本号。

## 5. MQTT Topic 设计

| Topic | 方向 | QoS | 说明 |
|-------|------|-----|------|
| `voice/up/audio` | 设备→服务器 | 0 | 上行音频 PCM 分片 |
| `voice/up/control` | 设备→服务器 | 1 | 上行控制信号（start/stop） |
| `voice/down/audio` | 服务器→设备 | 0 | 下行合成音频分片 |
| `voice/down/control` | 服务器→设备 | 1 | 下行控制信号（done/error） |

## 6. 上下行协议

### 上行（设备→服务器）

**控制消息** (`voice/up/control`)：
```json
...
ReadWritePaths=/home/water/voiceServer/logs/
PrivateTmp=true

# 环境变量（API Key 等敏感配置）
EnvironmentFile=/home/water/voiceServer/.env
...
```bash
# 创建日志目录
mkdir -p /home/water/voiceServer/logs

# 设置 .env 权限
chmod 600 /home/water/voiceServer/.env
...
# 常用操作
sudo systemctl status voice-server    # 查看状态
sudo systemctl restart voice-server   # 重启
sudo journalctl -u voice-server -f    # 实时日志
sudo journalctl -u voice-server --since "1 hour ago"  # 查看历史日志
```
...
| 参数 | 值 | 说明 |
|------|------|------|
| MemoryMax | 512M | 硬上限，超过则被 OOM kill 并自动重启 |
| MemoryHigh | 384M | 软上限，超过后内存分配被节流（swap 压力） |
| CPUQuota | 150% | 最多使用 1.5 核（2 核服务器留 0.5 核给系统和 EMQX） |
| RestartSec | 5 | 崩溃后 5 秒重启，防止频繁重启风暴 |

### 为什么用 systemd 而不是 nohup/screen

| 方式 | 问题 |
|------|------|
| `nohup python3 server.py &` | 崩溃不会自愈，无法限制资源，日志管理困难 |
| `screen/tmux` | 需要手动管理，服务器重启后不会自动恢复 |
| `systemd` | 开机自启、崩溃自愈、资源限制、日志自动归档（journald）、安全隔离 |

## 12. 附录

### 本地资源

- WM8978G 数据手册：`voiceServer/WM8978_v4.5-1141768.pdf`
- WS63 开发指南（本地）：`voiceServer/fbb_ws63/`

### 外部资源

- WS63 开发指南（在线）：https://docs.hisilicon.com/repos/fbb_ws63/zh-CN/master/index.html
- fbb_ws63 仓库：https://gitee.com/tao-yuxuan/fbb_ws63
- WM8978 + I2S 参考（STM32 平台，可移植思路）：https://stm32f407-tech-doc.readthedocs.io/en/latest/base/12I2S/I2S-wm8978-%E9%9F%B3%E4%B9%90%E6%92%AD%E6%94%BE.html

### 已知风险

| 风险 | 说明 | 应对 |
|------|------|------|
| SDK 无 WM8978 驱动 | fbb_ws63 只有通用 I2S 驱动，无 WM8978 专用驱动 | 需自行移植或编写 I2C 控制 + I2S 数据收发驱动 |
| Python 3.8 EOL | 部分新版 SDK 可能不兼容 | 锁定依赖版本，选用支持 3.8 的库 |
| Mimo TTS 可用性 | "限时免费"可能过期 | 准备备选 TTS 方案 |
| 服务器资源有限 | 2 核 3.7GB，不适合本地跑大模型 | ASR/LLM/TTS 全部调用云端 API |
